/**
 * Render 프로덕션 클러스터 기존 게스트 계정 데이터 삭제 스크립트
 * 용도: cluster0.6ovf2ru.mongodb.net 클러스터에서 기존 게스트 데이터 삭제
 * 
 * 사용법: MONGODB_URL="mongodb+srv://..." node delete_old_guest_data_render.js
 */

const mongoose = require('mongoose');
const readline = require('readline');

// Render의 MONGODB_URL 사용 (환경변수에서)
const mongodb_url = process.env.MONGODB_URL;

if (!mongodb_url) {
  console.error('❌ 오류: MONGODB_URL 환경변수가 설정되지 않았습니다!');
  console.error('사용법: MONGODB_URL="mongodb+srv://..." node delete_old_guest_data_render.js');
  process.exit(1);
}

console.log('📍 연결 URL (마스킹됨):', mongodb_url.replace(/:[^:]*@/, ':****@'));
